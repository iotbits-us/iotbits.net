...
export default () => {
  ...
  const renderButton = (executing, isSubmitting) => {
    if (errMsg) {
      return (
        <button className="btn btn-lg btn-outline-primary mt-3" type="submit" disabled>
          Error
        </button>
      );
    } if (msgSent) {
        ...
        {({ resetForm }) => (
          ...
          >
            ...
            <div className="form-group">
              <label className="col-form-label col-form-label-lg" htmlFor="username">
                Name
              </label>
            ...
            <Reaptcha
              ref={rcRef}
              sitekey="6LeOeeMUAAAAAHHXain-vZIUkxJN9EQn_QhSvKkj"
              data-netlify-recaptcha="true"
              onError={onError}
              onExpire={onExpire}
              onVerify={onVerify}
            ...
            <div className="m-2 col-form-label col-form-label-lg">
              ...
              <span className={`badge badge-${loaded ? 'success' : 'primary'} mx-2 p-2`}>
                loaded
              </span>
              <span className={`badge badge-${verified ? 'success' : 'primary'} mx-2 p-2`}>
                verified
              </span>
              {executing && <span className="badge badge-primary mx-2 p-2">executing</span>}
              {rcError && <span className="badge badge-primary mx-2 p-2">error</span>}
              {rcError && (
                <button
